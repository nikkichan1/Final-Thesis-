##############
####Main code to run the code, getting the pngs and save results
# s1: read the csv files in the directory of syntheticV2
# run the GEV moving window methods (GEV+GEVMWoutlier1+GEVMWoutlier2)
# ---> save the estimated 100yr floods in the directory of Results_100floods_GEV
# ---> save the png for each csv in the directory of syntheticV2
# ---> save the evaluation metric results in the EvaluationResults_GEVMWmethods
# Please Note. we use the moving window, but for each csv 137 windows only get
# statistical metric (e.g., MAE)
#
#
###############
###Load package
...
for (iii in 1:length(filesALL)){
  ...
  for (i in 1:length(filesName)){
    ...
    tryCatch({
      ...
      ### 
      write.csv(resall,file=paste0("./syntheticdataV2/Results_100floods_GEV/",LocationName, fnames),row.names = F)
      ...
      ggsave(filename=paste0(tagName,'_',i,".png",sep=""),path=paste0('./syntheticdataV2/',LocationName))
  ...
  write.csv(resultsALL,file=paste0("./syntheticdataV2/EvaluationResults_GEVMWmethods/",outputName, 
                                   ".csv"),row.names = F)
